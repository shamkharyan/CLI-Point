...
#include "controller/parsing/errors/UnknownCommandException.h"
#include "controller/parsing/errors/InvalidInputException.h"
...
std::unique_ptr<ACommand> Parser::parse()
{
	...
	if (cmdName.type != Token::Type::Word)
		throw InvalidInputException("Invalid command syntax");

	std::shared_ptr<ACommandFactory> factory = registry.getFactory(cmdName.value);
	if (!factory)
		throw UnknownCommandException(cmdName.value);
	...
	if (st == State::Error)
		throw InvalidInputException(errMsg.c_str());
